...
WSDL.prototype.isIgnoredNameSpace = function(ns) {
	if(this.options.forceNameSpaceAlias){
		return -1;
	} else {
...
WSDL.prototype.objectToXML = function(obj, name, nsPrefix, nsURI, isFirst, xmlnsAttr, schemaObject, nsContext) {
  ...
  if(self.options.forceNameSpaceAlias){ // OCS 
	  parentNsPrefix = nsPrefix;
  }
  ...
  if (Array.isArray(obj)) {
    for (i = 0, n = obj.length; i < n; i++) {
      ...
      if(self.options.forceNameSpaceAlias){
			  correctOuterNsPrefix = parentNsPrefix;
		  }	
