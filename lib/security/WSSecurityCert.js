...
var ejs = require('ejs');
// uses the signature algorithm 'http://www.w3.org/2001/04/xmldsig-more#rsa-sha512'
var SignedXml = require('xml-crypto').SignedXml(null,  { signatureAlgorithm : 'http://www.w3.org/2001/04/xmldsig-more#rsa-sha512'});
...
// Enhancement - Added Signature Prefix Capabilities
WSSecurityCert.prototype.postProcess = function (xml, envelopeKey, signaturePrefix) {
  ...
  var xmlWithSec = insertStr(secHeader, xml, xml.indexOf('</'+envelopeKey+':Header>')).replace(/[\r\n]/g, '');

  var references = [ "http://www.w3.org/2001/10/xml-exc-c14n#"];
  this.signer.addReference("//*[name(.)='wsse:Security']/*[local-name(.)='BinarySecurityToken']", references, 'http://www.w3.org/2001/04/xmlenc#sha256');
  this.signer.addReference("//*[name(.)='wsse:Security']/*[local-name(.)='Timestamp']", references, 'http://www.w3.org/2001/04/xmlenc#sha256');
  this.signer.addReference("//*[name(.)='com:identity']", references, 'http://www.w3.org/2001/04/xmlenc#sha256');
  this.signer.addReference("//*[name(.)='" + envelopeKey + ":Body']", references, 'http://www.w3.org/2001/04/xmlenc#sha256');
 
  this.signer.computeSignature(xmlWithSec, {prefix:signaturePrefix});

  return insertStr(this.signer.getSignatureXml(), xmlWithSec, xmlWithSec.indexOf('<wsse:BinarySecurityToken'));
};
