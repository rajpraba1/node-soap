...
// support for WSSecurityCert along with Client SSL Security at the same time
Client.prototype.setWSSecurity = function(wsSecurity) {
  this.wsSecurity = wsSecurity;
};

// support for Username/password digest along with Client SSL Security at the same time
Client.prototype.setWSUserTokenSecurityHeader = function(wsSecurity) {
  this.wsUserTokenSecurityHeader = wsSecurity;
 };
...
Client.prototype._invoke = function(method, args, location, callback, options, extraHeaders) {
  var self = this,
    ...
    xmlnsSoap = "xmlns:" + envelopeKey + "=\"http://schemas.xmlsoap.org/soap/envelope/\"";
  	...
  	input.signaturePrefix = "ds"; // Default Signature Prefix OCS

	//  ability to submit input option overrides when running the method
	if(options.input){
		for(attr in options.input){
			input[attr] = options.input[attr];	
		}
	}
	if(options.wsdl){
		for(attr in options.wsdl){
			this.wsdl.options[attr] = options.wsdl[attr];
		}
	}
  ...
  //bacward compatibility where just one of security handling at a time is supported
  if(!self.wsSecurity)
    self.wsSecurity = self.security;

  // support for WSSecurityCert along with Client SSL Security and Username/paswword security token
  xml = "<?xml version=\"1.0\" encoding=\"utf-8\"?>" +
   ...
   // "xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" " +
    ...
    ((self.soapHeaders || self.wsSecurity || wsUserTokenSecurityHeader) ? 
  //  ((self.soapHeaders || self.wsSecurity) ?
      (
        "<" + envelopeKey + ":Header>" +
        (self.soapHeaders ? self.soapHeaders.join("\n") : "") +
        (self.wsSecurity && !self.wsSecurity.postProcess ? self.wsSecurity.toXML() : "") +
        (self.wsUserTokenSecurityHeader && !self.wsUserTokenSecurityHeader.postProcess ? self.wsUserTokenSecurityHeader.toXML() : "") +
        "</" + envelopeKey + ":Header>"
      )
      :
        ''
    ...
    (self.wsSecurity && self.wsSecurity.postProcess ? ' Id="_0"' : '') +
    ">" +
    message +
    "</" + envelopeKey + ":Body>" +
    "</" + envelopeKey + ":Envelope>";
  ...
  if(self.wsSecurity && self.wsSecurity.postProcess){
    xml = self.wsSecurity.postProcess(xml, envelopeKey, input.signaturePrefix);
  }
